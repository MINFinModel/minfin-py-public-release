...
def _calc_equity_logic(inv_series, eirr, life, years):
    ...
    inv = inv_series.reindex(years, fill_value=0.0)
    for curr_year in years:
        start_window = curr_year - np.floor(life)
        past_inv = inv[(inv.index > start_window) & (inv.index <= curr_year)]
        flow[curr_year] = past_inv.sum() * eirr
    return flow


def _fx_factors(er, years, *, target_is_local, local_curr_code):
    """Per-year FC/LC conversion factors for repayment outputs."""
    er = er.loc[min(years):]
    lc_rate = er[local_curr_code]
    fc_rate = er["USD"]
    if target_is_local:
        conv_fc_to_target = lc_rate / fc_rate
        conv_lc_to_target = pd.Series(1.0, index=years)
    else:
        conv_fc_to_target = pd.Series(1.0, index=years)
        conv_lc_to_target = fc_rate / lc_rate
    return conv_fc_to_target.reindex(years), conv_lc_to_target.reindex(years)


def cal_interest_cost(
    tech_stats,
    er,
    ...
    conv_fc_to_target, conv_lc_to_target = _fx_factors(
        er, years, target_is_local=target_is_local, local_curr_code=local_curr_code
    )
    for src, config in tech_stats.financing_configs.items():
        ...
        if (inv_fc_debt.sum() + inv_lc_debt.sum()) >= 0:
            ...
            repay_fc = (
                _calc_debt_logic(inv_fc_debt, rate, term, grace, fin_share, years, interest_only=True)
                * conv_fc_to_target
            )
            repay_lc = (
                _calc_debt_logic(inv_lc_debt, rate, term, grace, fin_share, years, interest_only=True)
                * conv_lc_to_target
            )
            results[f"Loans ({src})"] = (repay_fc + repay_lc).reindex(years).fillna(0.0)

    return pd.DataFrame(results).T.sum()


def calculate_detailed_repayments(
    tech_stats,
    er,
    years=list(range(2025, 2071)),
    target_is_local=False,
    ...
    for src, config in tech_stats.financing_configs.items():
        ...
        if (inv_fc_debt.sum() + inv_lc_debt.sum()) >= 0:
            ...
            repay_fc = (
                _calc_debt_logic(inv_fc_debt, rate, term, grace, fin_share, years)
                * conv_fc_to_target
            )
            repay_lc = (
                _calc_debt_logic(inv_lc_debt, rate, term, grace, fin_share, years)
                * conv_lc_to_target
            )
            results[f"Loans ({src})"] = (repay_fc + repay_lc).reindex(years).fillna(0.0)
        ...
        if (inv_fc_equity.sum() + inv_lc_equity.sum()) >= 0:
            ...
            results[f"Equity ({src})"] = (return_fc + return_lc).reindex(years).fillna(0.0)
